...
#Spiked QC recovery was calculated with target of 0.1ppm
target_recovery = (~ ./0.1*(100))
...
avg_plot1<- max_min_spiked_ocp %>%
  ...
  geom_hline(data = ocp_hline, aes(yintercept = ocp_hline$mean,linetype = factor(ocp_hline$linetype_mean), show_guide = TRUE),color = "blue", size=0.6)+
...
avg_plot2<- max_min_spiked_pcb %>%
  ...
  geom_point(aes(color=`Batch_num`)) +
    geom_label_repel(data = pcb_hline, 
                  aes(x = 5, y = pcb_hline$mean + 0.001, label = paste0(average_symbol, "=", sprintf("%.3f",pcb_hline$mean))),#sig figs
                  size=3, #Do not repel labels from data points
                  min.segment.length = unit(0, 'lines'),
                      arrow = arrow(length = unit(0.01, "npc")),box.padding = 3
    )+ 
...
PCB_QC_recovery_mean_rounded %>% 
  ggplot(aes(x=n, y=name)) +
    ...
    geom_vline(aes(),xintercept = 100, linetype="dashed", 
                color = "blue", size=1)+
    ggtitle("Spiked average recovery % per batch - PCB") + 
    theme(plot.title = element_text(hjust = 0.5)) +
  theme(legend.key.height= unit(5, 'cm'),
        legend.key.width= unit(5, 'cm'),
        legend.title = element_text(size=25),
...
ocp1<-OCPPBDE_QC_recovery %>% 
  group_by(HCB) %>% 
    ...
    annotate("text",x=15, y=130, size=4, label = paste("n=",ocp_qc_batches$`n (per analyte)`[1] ))
ggplotly(ocp1)
...
pcb1<-ggplot(PCB_QC_recovery, aes(factor(Batch_num), `PCB-118 `, color=PCB_QC_recovery$Batch_number)) +
    ...
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
    theme(legend.position = "none")+
    annotate("text",x=25, y=110, size=4, label = paste("n=",pcb_qc_batches$`n (per analyte)`[1] ))
...
ocp_qc_batches%>% 
  ...
  theme(text = element_text(colour = vit_c_palette["light_text"],family = "Cabin"),
        ...
        strip.text = element_text(family = "Enriqueta",
                                  colour = vit_c_palette["light_text"], face = "bold"),
        panel.margin.x = unit(1,"lines"), panel.margin.y = unit(0,"lines")
  )
...
```{r QC plot - unspiked PCB, echo=FALSE,message=FALSE,warning=FALSE, fig.height = 20, fig.width =10 }
...
PCB_QC_unspike_mean %>%
    ...
    ggplot(aes(n, name)) +
    geom_col() +
    facet_wrap(~Batch_number, ncol=3)+
    labs(y= "Analytes", x = "Concentration")+
...
shewhart_pcb_unspike<-max_min_unspike_pcb %>%
  ...
  geom_label_repel(data = pcb_unspike_hline, 
                  ...
                  size=3, #Do not repel labels from data points
                  min.segment.length = unit(0, 'lines'),
                      arrow = arrow(length = unit(0.01, "npc")),box.padding = 2,
                  nudge_x = -0.001
    )+ 
...
avg_plot1<- max_min_blanks_ocp %>%
  ...
  geom_label_repel(data = ocp_blanks_hline, 
                  aes(x = 4, y = ocp_blanks_hline$mean, label = paste0(average_symbol, "=", sprintf("%.3f",ocp_blanks_hline$mean))),#sig figs
                  size=3, #Do not repel labels from data points
...
stats_OCP %>%
    kbl(
    caption = "<center><span style='font-size:30px'>OCP: Samples - Descriptive Statistics </span></center>") %>% 
...
IQR_OCPPBDE %>% 
  kbl(
    caption = "<center><span style='font-size:30px'>OCP Samples - IQR </span></center>") %>%
...
stats_PCB %>%
    kbl(
    caption = "<center><span style='font-size:30px'>PCB Samples - Descriptive Statistics </span></center>") %>% 
    add_header_above(c(" " = 1, "2T" = 3, "3T" =3, "Post" = 3)) %>% 
  kable_styling( stripe_color = "gray!6",
    bootstrap_options = c("striped", "hover")) %>% 
    scroll_box(width = "1000px", height = "500px")
...
IQR_PCB %>% 
  kbl(
    caption = "<center><span style='font-size:30px'>PCB Samples - IQR </span></center>") %>%
