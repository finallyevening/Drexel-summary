...
shewhart_ocp_unspike<-ggplot(max_min_unspike_ocp, aes(x = factor(Batch_num), y = value)) + 
  facet_wrap(~variable,ncol=1, scales="free_y")+
    geom_blank(aes(y = y_min)) +
    geom_blank(aes(y = y_max))+
  geom_line(aes(color=`Batch_num`)) + 
  geom_point(aes(color=`Batch_num`)) + 
    geom_hline(data = ocp_unspike_hline, aes(yintercept = `mean`, linetype= `linetype_mean`), color = "blue", size=0.6)+
  ...
  theme(axis.title.y = element_text(size=10, face="bold", colour = "black"),
        ...
        strip.text = element_text(face = "bold", size=10))+
  labs(y="Concentration", x = "Batch number")+
  ggtitle("QC Unspike: OCP distribution")
  
```


### Shewhart Chart: OCP (Unspiked)
```{r Display Unspiked OCP Shewhart Chart, echo=FALSE,message=FALSE,warning=FALSE,  fig.height = 40, fig.width =10}
...
shewhart_pcb_unspike<-ggplot(max_min_unspike_pcb, aes(x = factor(Batch_num), y = value, color=Batch_num)) + 
  facet_wrap(~variable,ncol=1, scales="free_y")+
    geom_blank(aes(y = y_min)) +
    geom_blank(aes(y = y_max))+
  geom_line(aes(color=`Batch_num`)) + 
  geom_point(aes(fill = Batch_num)) + 
  ...
  theme(axis.title.y = element_text(size=10, face="bold", colour = "black"),
        ...
        strip.text = element_text(face = "bold", size=10))+
  labs(y="Concentration", x = "Batch number")+
  ggtitle("QC Unspike: PCB distribution")
  theme(plot.title = element_text(hjust = 0.5))

```

```{r Display PCB Unspike, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 40, fig.width =10}
