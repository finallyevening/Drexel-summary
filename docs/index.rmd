...
ocp_batches %>%
  ...
  geom_boxplot( aes(alpha = value),lwd=1.25,outlier.colour="red", outlier.shape=19,
                outlier.size=5)+
  annotate("text",  x=16, y=30, size=8, label = paste("n=",ocp_batches$`n (per analyte)`[1] ))+
  facet_wrap(~var, scales = "free",ncol=1)+
  labs(y="Concentration (log10)", x = "Batch number", 
       title =  paste0("Batch Effect Evaluation: OCP"),
       subtitle = "Sample concentration grouped by analyte across all batches ") +
...
ocp_batches %>%
  ...
  theme(legend.key.height= unit(5, 'cm'),
    ...
    annotate("text",x=15, y=0.01, size=8, label = paste("n=",ocp_batches$`n (per analyte)`[1] ))
```
...
```{r HCB plot: Running average distribution, echo=FALSE,message=FALSE,warning=FALSE,  fig.height = 25, fig.width =8}
...
ocp_hline <- data.frame(
  ...
  linetype_mean = c("dashed"),
  linetype_sd1 = c("dotdash"),
  linetype_sd_1 = c("dotdash"),
  linetype_sd2= c("twodash"),
  linetype_sd_2= c("dashed"),
