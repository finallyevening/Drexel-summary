...
df2 %>%
  select('Lab sample ID','Tooth Powder','Fraction.x','Drill Round #','Extraction Batch','Extraction Label','Extraction Date','Cleanup date','#') %>% 
  kbl(
...
analytes<-rownames(OCPPBDE_QC_recovery_mean)
batches<-colnames(OCPPBDE_QC_recovery_mean)
```
...
OCPPBDE_QC_Spiked %>%
  ...
  kable_styling( stripe_color = "gray!6",
    bootstrap_options = c("striped", "hover")) %>% 
    scroll_box(width = "2000px", height = "500px")
...
q<-notes_OCPPBDE %>% 
  select("Stage","HCB Results","Oxychlordane Results","trans-Nonachlor Results","pp-DDE Results","pp-DDT Results","Tooth Powder","Start Drill date", "Batch_num") %>% 
  group_by("Batch_num") %>% 
...
s<-notes_OCPPBDE %>% 
  select("Stage","HCB Results","Oxychlordane Results","trans-Nonachlor Results","pp-DDT Results","pp-DDE Results","Tooth Powder","Start Drill date", "Batch_num") %>%
  group_by("Batch_num") %>%
...
ocp_batches %>% 
  ...
  geom_boxplot(lwd=1.25,outlier.colour="red", outlier.shape=19,
                outlier.size=5)+
  facet_wrap(~var, scales = "free",ncol=1)+
  labs(y= "Concentration (log10)", x = "Batch number")+
  scale_fill_brewer(palette="Set3")+
  scale_y_continuous(trans='log10')+
  ggtitle("Batch Effect Evaluation - OCP/PBDE, by batch number") + 
...
pcb_batches %>% 
  ...
  scale_fill_brewer(palette="Set3")+
  scale_y_continuous(trans='log10')+
  ggtitle("Batch Effect Evaluation - PCB, by batch number") + 
  theme(legend.key.height= unit(5, 'cm'),
        legend.key.width= unit(5, 'cm'),
        legend.title = element_text(size=25),
        legend.text = element_text(size=25),
...
```{r Hlines OCP, echo=FALSE, message=FALSE, warning=FALSE}

add_label_line_mean <- function(y, x, text = paste("running average")) {
  list(geom_hline(yintercept = y),
       annotate("text", x = mean, y = y, label = text, vjust = -0.5, hjust=-0.5)
  )
}

add_label_line_sd <- function(y, x, text = paste("Standard Deviation")) {
  list(geom_hline(yintercept = y),
       annotate("text", x = mean+sd, y = y, label = text, vjust = -0.5)
  )
}
...
#HCB
mean_hcb<-mean(OCPPBDE_QC_Spiked$`HCB Results`)
sd_hcb<-sd(OCPPBDE_QC_Spiked$`HCB Results`)
...
#b-HCH
mean_bHCH<-mean(OCPPBDE_QC_Spiked$`b-HCH Results`)
sd_bHCH<-sd(OCPPBDE_QC_Spiked$`b-HCH Results`)

bHCH_hline <- data.frame(
  y = c(mean_bHCH, mean_bHCH + c(1, -1, 2, -2) * sd_bHCH),
  Descriptive_Statistics = c("Running Average", "+1*SD ", "-1*SD ", "+2*SD", "-2*SD"),
  linetype = c("solid", "dashed", "dashed", "dotted", "dotted")
)

#Oxychlordane
mean_Oxychlordane<-mean(OCPPBDE_QC_Spiked$`Oxychlordane Results`)
sd_Oxychlordane<-sd(OCPPBDE_QC_Spiked$`Oxychlordane Results`)

Oxychlordane_hline <- data.frame(
  y = c(mean_Oxychlordane, mean_Oxychlordane + c(1, -1, 2, -2) * sd_Oxychlordane),
  Descriptive_Statistics = c("Running Average", "+1*SD ", "-1*SD ", "+2*SD", "-2*SD"),
  linetype = c("solid", "dashed", "dashed", "dotted", "dotted")
)
...
#trans-Nonachlor
mean_trans_Nonachlor<-mean(OCPPBDE_QC_Spiked$`trans-Nonachlor Results`)
sd_trans_Nonachlor<-sd(OCPPBDE_QC_Spiked$`trans-Nonachlor Results`)
...
#pp_DDE
mean_pp_DDE<-mean(OCPPBDE_QC_Spiked$`pp-DDE Results`)
sd_pp_DDE<-sd(OCPPBDE_QC_Spiked$`pp-DDE Results`)
...
#pp-DDE-2 Results
mean_pp_DDE_2<-mean(OCPPBDE_QC_Spiked$`pp-DDE-2 Results`)
sd_pp_DDE_2<-sd(OCPPBDE_QC_Spiked$`pp-DDE-2 Results`)

pp_DDE_2_hline <- data.frame(
  y = c(mean_pp_DDE_2, mean_pp_DDE_2 + c(1, -1, 2, -2) * sd_pp_DDE_2),
  Descriptive_Statistics = c("Running Average", "+1*SD ", "-1*SD ", "+2*SD", "-2*SD"),
  linetype = c("solid", "dashed", "dashed", "dotted", "dotted")
)

#BDE-28 Results
mean_BDE_28<-mean(OCPPBDE_QC_Spiked$`BDE-28 Results`)
sd_BDE_28<-sd(OCPPBDE_QC_Spiked$`BDE-28 Results`)

BDE_28_hline <- data.frame(
  y = c(mean_BDE_28, mean_BDE_28 + c(1, -1, 2, -2) * sd_BDE_28),
  Descriptive_Statistics = c("Running Average", "+1*SD ", "-1*SD ", "+2*SD", "-2*SD"),
  linetype = c("solid", "dashed", "dashed", "dotted", "dotted")
)
...
#pp-DDT Results
mean_pp_DDT<-mean(OCPPBDE_QC_Spiked$`pp-DDT Results`)
sd_pp_DDT<-sd(OCPPBDE_QC_Spiked$`pp-DDT Results`)
...
#pp-DDT-2 Results
mean_pp_DDT_2<-mean(OCPPBDE_QC_Spiked$`pp-DDT-2 Results`)
sd_pp_DDT_2<-sd(OCPPBDE_QC_Spiked$`pp-DDT-2 Results`)

pp_DDT_2_hline <- data.frame(
  y = c(mean_pp_DDT_2, mean_pp_DDT_2 + c(1, -1, 2, -2) * sd_pp_DDT_2),
  Descriptive_Statistics = c("Running Average", "+1*SD ", "-1*SD ", "+2*SD", "-2*SD"),
  linetype = c("solid", "dashed", "dashed", "dotted", "dotted")
)

#BDE-47 Results
mean_BDE_47<-mean(OCPPBDE_QC_Spiked$`BDE-47 Results`)
sd_BDE_47<-sd(OCPPBDE_QC_Spiked$`BDE-47 Results`)
...
#BDE-100 Results
mean_BDE_100<-mean(OCPPBDE_QC_Spiked$`BDE-100 Results`)
sd_BDE_100<-sd(OCPPBDE_QC_Spiked$`BDE-100 Results`)
...
#BDE-99 Results
mean_BDE_99<-mean(OCPPBDE_QC_Spiked$`BDE-99 Results`)
sd_BDE_99<-sd(OCPPBDE_QC_Spiked$`BDE-99 Results`)
...
#BDE-153 Results
mean_BDE_153<-mean(OCPPBDE_QC_Spiked$`BDE-153 Results`)
sd_BDE_153<-sd(OCPPBDE_QC_Spiked$`BDE-153 Results`)

BDE_153_hline <- data.frame(
  y = c(mean_BDE_153, mean_BDE_153 + c(1, -1, 2, -2) * sd_BDE_99),
  Descriptive_Statistics = c("Running Average", "+1*SD ", "-1*SD ", "+2*SD", "-2*SD"),
  linetype = c("solid", "dashed", "dashed", "dotted", "dotted")
)
```
...
```{r HCB plot: Running average distribution, echo=FALSE,message=FALSE,warning=FALSE,  fig.height = 100, fig.width =15}
...
avg_plot1<-ggplot(max_min_spiked_ocp, aes(x = factor(Batch_num), y = value)) + 
  facet_wrap(~variable,ncol=1, scales="free_y")+
    geom_blank(aes(y = y_min)) +
    geom_blank(aes(y = y_max))+
  geom_line(aes(color=`Batch_num`)) + 
  geom_point(aes(color=`Batch_num`)) + 
    geom_hline(data = stats, aes(yintercept = mean), linetype = 1, color = "blue", size=0.5)+
    geom_hline(data = stats, aes(yintercept = `sd+1`),linetype = 5, color = "black", size=0.2)+
    geom_hline(data = stats, aes(yintercept = `sd-1`),linetype = 5, color = "black", size=0.2)+
    geom_hline(data = stats, aes(yintercept = `sd+2`),linetype = 4, color = "black", size=0.2)+
    geom_hline(data = stats, aes(yintercept = `sd-2`),linetype = 4, color = "black", size=0.2)+
  guides(color="none")+
  theme(axis.title.y = element_text(size=10, face="bold", colour = "black"),
        axis.text.y = element_text(size=10, face="bold", colour = "black"),
        strip.text = element_text(face = "bold", size=10))+
  labs(y="Concentration", x = "Batch number",linetype = c("+/- 1 SD","+/- 1 SD","+/- 1 SD", "+/- 2 SD", "Running Average"))+
  ggtitle("QC : HCB distribution")+
  theme(plot.title = element_text(hjust = 0.5))+
  scale_linetype_manual(name = "limit",
                        values = c(1,2,3,4,5),
                        labels = c("+/- 1 SD","+/- 1 SD","+/- 1 SD", "+/- 2 SD", "Running Average",
...
```{r pp-DDE plot: Running average distribution, echo=FALSE,message=FALSE,warning=FALSE}
avg_plot5<-ggplot(
  OCPPBDE_QC_Spiked, aes(x = factor(Batch_num), y = `pp-DDE Results`, group=`Batch_num`)) + 
  ...
  ggtitle("QC:pp-DDE distribution")+
...
```{r pp-DDT Results plot: Running average distribution, echo=FALSE,message=FALSE,warning=FALSE}

avg_plot8<-ggplot(
  OCPPBDE_QC_Spiked, aes(x = factor(Batch_num), y = `pp-DDT Results`, group=`Batch_num`)) + 
  geom_line(aes(color=`Batch_number`)) + 
  geom_point(aes(color=`Batch_number`)) + 
  guides(color="none")+
  ...
  theme(axis.title.y = element_text(size=14, face="bold", colour = "black"),
        ...
        strip.text = element_text(face = "bold", size=14))+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  ggtitle("QC: pp-DDT distribution")+
  labs(y="Concentration", x = "Batch number",linetype = "Control Chart" )+
  theme(plot.title = element_text(hjust = 0.5))+
  scale_linetype_manual(values = c(1,2,3),
                        labels = c("+/- 1 SD", "+/- 2 SD", "Running Average"))
...
### Recovery (%) Across Batches: OCP (Spiked)
```{r QC plot - OCP spiked mean, echo=FALSE,message=FALSE,warning=FALSE, fig.height = 120, fig.width = 50 }


OCPPBDE_QC_recovery_mean_rounded<-OCPPBDE_QC_recovery_mean %>%
  select("HCB","Oxychlordane","Oxychlordane","trans-Nonachlor", "pp-DDE","pp-DDT","Batch_num") %>% 
    group_by(Batch_num) %>% 
...
### Recovery (%) Across Batches: PCB (Spiked)
...
### Recovery(%) by Analyte: OCP (Spiked)

```{r OCP spiked batch cleanup, echo=FALSE,message=FALSE,warning=FALSE}
OCPPBDE_QC_recovery<-OCPPBDE_QC_recovery %>% 
  mutate(Batch_num=str_sub(`Batch_number`,4,5))
...
#HCB
ocp1<-ggplot(OCPPBDE_QC_recovery, aes(factor(Batch_num), HCB, color=OCPPBDE_QC_recovery$Batch_number)) +
    geom_point() +
...
grid.arrange(ocp1, ocp3, ncol=2)
grid.arrange(ocp4, ocp5, ncol=2)
grid.arrange(ocp7, ncol=1)


```

```{r PCB spiked batch cleanup, echo=FALSE,message=FALSE,warning=FALSE}
PCB_QC_recovery<-PCB_QC_recovery %>% 
  mutate(Batch_num=str_sub(`Batch_number`,4,5))
...
### Recovery(%) by Analyte : PCB (Spiked)
...
pcb1<-ggplot(PCB_QC_recovery, aes(factor(Batch_num), `PCB-118 `, color=PCB_QC_recovery$Batch_number)) +
    geom_point() +
    labs(y= "Recovery % [log10]", x = "Batch Number")+
    geom_hline(yintercept = 100, linetype="dashed", 
                color = "blue")+
    scale_y_continuous(trans='log10')+
...
grid.arrange(pcb1, pcb2, ncol=2)
grid.arrange(pcb3, pcb4, ncol=2)
grid.arrange(pcb5, pcb6, ncol=2)
grid.arrange(pcb7, ncol=2)
```
...
#Oxychlordane
mean_Oxychlordane<-mean(OCPPBDE_QC_Unspike$`Oxychlordane Results`)
sd_Oxychlordane<-sd(OCPPBDE_QC_Unspike$`Oxychlordane Results`)
...
#pp-DDE-2 Results
mean_pp_DDE_2<-mean(OCPPBDE_QC_Unspike$`pp-DDE-2 Results`)
sd_pp_DDE_2<-sd(OCPPBDE_QC_Unspike$`pp-DDE-2 Results`)
...
```{r UNSPIKED OCP: Oxychlordane plot - Running average distribution, echo=FALSE,message=FALSE,warning=FALSE}

avg_plot22<-ggplot(OCPPBDE_QC_Unspike, aes(x = factor(Batch_num), y = `Oxychlordane Results`, group=`Batch_num`)) + 
  geom_line(aes(color=`Batch_number`)) + 
  geom_point(aes(color=`Batch_number`)) + 
  guides(color="none")+
  geom_hline(data = Oxychlordane_hline, aes(yintercept = y , linetype = `linetype`), color = "black", size=0.5)+
   theme(axis.title.y = element_text(size=10, face="bold", colour = "black"),
        axis.text.y = element_text(size=10, face="bold", colour = "black"),
        strip.text = element_text(face = "bold", size=10))+
  labs(y="Concentration", x = "Batch number",linetype = "Control Chart" )+
  ggtitle("Unspiked QC : Oxychlordane distribution")+
  theme(plot.title = element_text(hjust = 0.5))+
  scale_linetype_manual(values = c(1,2,3),
                        labels = c("+/- 1 SD", "+/- 2 SD", "Running Average"))
...
```{r UNSPIKED OCP: pp-DDE-2 Results plot - Running average distribution, echo=FALSE,message=FALSE,warning=FALSE}

avg_plot44<-ggplot(OCPPBDE_QC_Unspike, aes(x = factor(Batch_num), y = `pp-DDE-2 Results`, group=`Batch_num`)) + 
  geom_line(aes(color=`Batch_number`)) + 
  geom_point(aes(color=`Batch_number`)) + 
  guides(color="none")+
  geom_hline(data = pp_DDE_2_hline, aes(yintercept = y , linetype = `linetype`), color = "black", size=0.5)+
   theme(axis.title.y = element_text(size=10, face="bold", colour = "black"),
        axis.text.y = element_text(size=10, face="bold", colour = "black"),
        strip.text = element_text(face = "bold", size=10))+
  labs(y="Concentration", x = "Batch number",linetype = "Control Chart" )+
  ggtitle("Unspiked QC : pp-DDE-2 distribution")+
  theme(plot.title = element_text(hjust = 0.5))+
  scale_linetype_manual(values = c(1,2,3),
                        labels = c("+/- 1 SD", "+/- 2 SD", "Running Average"))
...
### Concentration across batches: PCB (Unspiked)
...
```{r Hlines PCB Unspiked, echo=FALSE, message=FALSE, warning=FALSE}
#PCB-118  Results
mean_pcb_118<-mean(PCB_QC_Unspike$`PCB-118  Results`)
sd_pcb_118<-sd(PCB_QC_Unspike$`PCB-118  Results`)

pcb_118_hline <- data.frame(
  y = c(mean_pcb_118, mean_pcb_118 + c(1, -1, 2, -2) * sd_pcb_118),
  Descriptive_Statistics = c("Running Average", "+1*SD ", "-1*SD ", "+2*SD", "-2*SD"),
  linetype = c("solid", "dashed", "dashed", "dotted", "dotted")
)

#PCB-153 Results
mean_pcb_153<-mean(PCB_QC_Unspike$`PCB-153 Results`)
sd_pcb_153<-sd(PCB_QC_Unspike$`PCB-153 Results`)

pcb_153_hline <- data.frame(
  y = c(mean_pcb_153, mean_pcb_153 + c(1, -1, 2, -2) * sd_pcb_153),
  Descriptive_Statistics = c("Running Average", "+1*SD ", "-1*SD ", "+2*SD", "-2*SD"),
  linetype = c("solid", "dashed", "dashed", "dotted", "dotted")
)
...
#PCB-138 Results
mean_pcb_138<-mean(PCB_QC_Unspike$`PCB-138 Results`)
sd_pcb_138<-sd(PCB_QC_Unspike$`PCB-138 Results`)

pcb_138_hline <- data.frame(
  y = c(mean_pcb_138, mean_pcb_138 + c(1, -1, 2, -2) * sd_pcb_138),
  Descriptive_Statistics = c("Running Average", "+1*SD ", "-1*SD ", "+2*SD", "-2*SD"),
  linetype = c("solid", "dashed", "dashed", "dotted", "dotted")
)
...
#PCB-156 Results
mean_pcb_156<-mean(PCB_QC_Unspike$`PCB-156 Results`)
sd_pcb_156<-sd(PCB_QC_Unspike$`PCB-156 Results`)

pcb_156_hline <- data.frame(
  y = c(mean_pcb_156, mean_pcb_156 + c(1, -1, 2, -2) * sd_pcb_156),
  Descriptive_Statistics = c("Running Average", "+1*SD ", "-1*SD ", "+2*SD", "-2*SD"),
  linetype = c("solid", "dashed", "dashed", "dotted", "dotted")
)
...
#PCB-157 Results
mean_pcb_157<-mean(PCB_QC_Unspike$`PCB-157 Results`)
sd_pcb_157<-sd(PCB_QC_Unspike$`PCB-157 Results`)

pcb_157_hline <- data.frame(
  y = c(mean_pcb_157, mean_pcb_157 + c(1, -1, 2, -2) * sd_pcb_157),
  Descriptive_Statistics = c("Running Average", "+1*SD ", "-1*SD ", "+2*SD", "-2*SD"),
  linetype = c("solid", "dashed", "dashed", "dotted", "dotted")
)
...
#PCB-180 Results
mean_pcb_180<-mean(PCB_QC_Unspike$`PCB-180 Results`)
sd_pcb_180<-sd(PCB_QC_Unspike$`PCB-180 Results`)

pcb_180_hline <- data.frame(
  y = c(mean_pcb_180, mean_pcb_180 + c(1, -1, 2, -2) * sd_pcb_180),
  Descriptive_Statistics = c("Running Average", "+1*SD ", "-1*SD ", "+2*SD", "-2*SD"),
  linetype = c("solid", "dashed", "dashed", "dotted", "dotted")
)
...
#PCB-170 Results
mean_pcb_170<-mean(PCB_QC_Unspike$`PCB-170 Results`)
sd_pcb_170<-sd(PCB_QC_Unspike$`PCB-170 Results`)

pcb_170_hline <- data.frame(
  y = c(mean_pcb_170, mean_pcb_170 + c(1, -1, 2, -2) * sd_pcb_170),
  Descriptive_Statistics = c("Running Average", "+1*SD ", "-1*SD ", "+2*SD", "-2*SD"),
  linetype = c("solid", "dashed", "dashed", "dotted", "dotted")
)
```
...
### Shewhart Chart: PCB (Unspiked)
```{r UNSPIKED PCB: PCB-118 plot - Running average distribution, echo=FALSE,message=FALSE,warning=FALSE}

avg_plot111<-ggplot(PCB_QC_Unspike, aes(x = factor(Batch_num), y = `PCB-118  Results`, group=`Batch_num`)) + 
  geom_line(aes(color=`Batch_number`)) + 
  geom_point(aes(color=`Batch_number`)) + 
  guides(color="none")+
  geom_hline(data = pcb_118_hline, aes(yintercept = y , linetype = `linetype`), color = "black", size=0.5)+
   theme(axis.title.y = element_text(size=10, face="bold", colour = "black"),
        axis.text.y = element_text(size=10, face="bold", colour = "black"),
        strip.text = element_text(face = "bold", size=10))+
  labs(y="Concentration", x = "Batch number",linetype = "Control Chart" )+
  ggtitle("Unspiked QC : PCB-118 distribution")+
  theme(plot.title = element_text(hjust = 0.5))+
  scale_linetype_manual(values = c(1,2,3),
                        labels = c("+/- 1 SD", "+/- 2 SD", "Running Average"))
...
```{r UNSPIKED PCB: PCB-153 plot - Running average distribution, echo=FALSE,message=FALSE,warning=FALSE}

avg_plot222<-ggplot(PCB_QC_Unspike, aes(x = factor(Batch_num), y = `PCB-153 Results`, group=`Batch_num`)) + 
  geom_line(aes(color=`Batch_number`)) + 
  geom_point(aes(color=`Batch_number`)) + 
  guides(color="none")+
  geom_hline(data = pcb_153_hline, aes(yintercept = y , linetype = `linetype`), color = "black", size=0.5)+
   theme(axis.title.y = element_text(size=10, face="bold", colour = "black"),
        axis.text.y = element_text(size=10, face="bold", colour = "black"),
        strip.text = element_text(face = "bold", size=10))+
  labs(y="Concentration", x = "Batch number",linetype = "Control Chart" )+
  ggtitle("Unspiked QC : PCB-153 distribution")+
  theme(plot.title = element_text(hjust = 0.5))+
  scale_linetype_manual(values = c(1,2,3),
                        labels = c("+/- 1 SD", "+/- 2 SD", "Running Average"))
...
```{r UNSPIKED PCB: PCB-138 plot - Running average distribution, echo=FALSE,message=FALSE,warning=FALSE}

avg_plot333<-ggplot(PCB_QC_Unspike, aes(x = factor(Batch_num), y = `PCB-138 Results`, group=`Batch_num`)) + 
  geom_line(aes(color=`Batch_number`)) + 
  geom_point(aes(color=`Batch_number`)) + 
  guides(color="none")+
  geom_hline(data = pcb_138_hline, aes(yintercept = y , linetype = `linetype`), color = "black", size=0.5)+
   theme(axis.title.y = element_text(size=10, face="bold", colour = "black"),
        axis.text.y = element_text(size=10, face="bold", colour = "black"),
        strip.text = element_text(face = "bold", size=10))+
  labs(y="Concentration", x = "Batch number",linetype = "Control Chart" )+
  ggtitle("Unspiked QC : PCB-138 distribution")+
  theme(plot.title = element_text(hjust = 0.5))+
  scale_linetype_manual(values = c(1,2,3),
                        labels = c("+/- 1 SD", "+/- 2 SD", "Running Average"))
...
```{r UNSPIKED PCB: PCB-156 Results plot - Running average distribution, echo=FALSE,message=FALSE,warning=FALSE}

avg_plot444<-ggplot(PCB_QC_Unspike, aes(x = factor(Batch_num), y = `PCB-156 Results`, group=`Batch_num`)) + 
  geom_line(aes(color=`Batch_number`)) + 
  geom_point(aes(color=`Batch_number`)) + 
  guides(color="none")+
  geom_hline(data = pcb_156_hline, aes(yintercept = y , linetype = `linetype`), color = "black", size=0.5)+
   theme(axis.title.y = element_text(size=10, face="bold", colour = "black"),
        axis.text.y = element_text(size=10, face="bold", colour = "black"),
        strip.text = element_text(face = "bold", size=10))+
  labs(y="Concentration", x = "Batch number",linetype = "Control Chart" )+
  ggtitle("Unspiked QC : PCB-156 distribution")+
  theme(plot.title = element_text(hjust = 0.5))+
  scale_linetype_manual(values = c(1,2,3),
                        labels = c("+/- 1 SD", "+/- 2 SD", "Running Average"))
...
```{r UNSPIKED PCB: PCB-157 Results plot - Running average distribution, echo=FALSE,message=FALSE,warning=FALSE}

avg_plot555<-ggplot(PCB_QC_Unspike, aes(x = factor(Batch_num), y = `PCB-157 Results`, group=`Batch_num`)) + 
  geom_line(aes(color=`Batch_number`)) + 
  geom_point(aes(color=`Batch_number`)) + 
  guides(color="none")+
  geom_hline(data = pcb_157_hline, aes(yintercept = y , linetype = `linetype`), color = "black", size=0.5)+
   theme(axis.title.y = element_text(size=10, face="bold", colour = "black"),
        axis.text.y = element_text(size=10, face="bold", colour = "black"),
        strip.text = element_text(face = "bold", size=10))+
  labs(y="Concentration", x = "Batch number",linetype = "Control Chart" )+
  ggtitle("Unspiked QC : PCB-157 distribution")+
  theme(plot.title = element_text(hjust = 0.5))+
  scale_linetype_manual(values = c(1,2,3),
                        labels = c("+/- 1 SD", "+/- 2 SD", "Running Average"))
...
```{r UNSPIKED PCB: PCB-180 Results plot - Running average distribution, echo=FALSE,message=FALSE,warning=FALSE}

avg_plot666<-ggplot(PCB_QC_Unspike, aes(x = factor(Batch_num), y = `PCB-180 Results`, group=`Batch_num`)) + 
  geom_line(aes(color=`Batch_number`)) + 
  geom_point(aes(color=`Batch_number`)) + 
  guides(color="none")+
  geom_hline(data = pcb_180_hline, aes(yintercept = y , linetype = `linetype`), color = "black", size=0.5)+
   theme(axis.title.y = element_text(size=10, face="bold", colour = "black"),
        axis.text.y = element_text(size=10, face="bold", colour = "black"),
        strip.text = element_text(face = "bold", size=10))+
  labs(y="Concentration", x = "Batch number",linetype = "Control Chart" )+
  ggtitle("Unspiked QC : PCB-180 distribution")+
  theme(plot.title = element_text(hjust = 0.5))+
  scale_linetype_manual(values = c(1,2,3),
                        labels = c("+/- 1 SD", "+/- 2 SD", "Running Average"))
...
```{r UNSPIKED PCB: PCB-170 Results plot - Running average distribution, echo=FALSE,message=FALSE,warning=FALSE}

avg_plot777<-ggplot(PCB_QC_Unspike, aes(x = factor(Batch_num), y = `PCB-170 Results`, group=`Batch_num`)) + 
  geom_line(aes(color=`Batch_number`)) + 
  geom_point(aes(color=`Batch_number`)) + 
  guides(color="none")+
  geom_hline(data = pcb_170_hline, aes(yintercept = y , linetype = `linetype`), color = "black", size=0.5)+
   theme(axis.title.y = element_text(size=10, face="bold", colour = "black"),
        axis.text.y = element_text(size=10, face="bold", colour = "black"),
        strip.text = element_text(face = "bold", size=10))+
  labs(y="Concentration", x = "Batch number",linetype = "Control Chart" )+
  ggtitle("Unspiked QC : PCB-170 distribution")+
  theme(plot.title = element_text(hjust = 0.5))+
  scale_linetype_manual(values = c(1,2,3),
                        labels = c("+/- 1 SD", "+/- 2 SD", "Running Average"))
...
```{r Display - Running average distribution, echo=FALSE,message=FALSE,warning=FALSE}
avg_plot111
avg_plot222
avg_plot333
avg_plot444
