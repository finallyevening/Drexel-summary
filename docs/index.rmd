...
```{r PCB QC SCatterplot - display plot, all samples, echo=FALSE,message=FALSE,warning=FALSE, fig.height = 30, fig.width =10 }
pcb_qc_batches %>%
group_by(Batch_num) %>% 
  ggplot(aes(x = factor(Batch_num2), y = value)) + 
  facet_wrap(~variable, scales="free",ncol=1)+
