...
ocp_batches %>%
  ...
  geom_boxplot( aes(alpha = value),lwd=1.25,outlier.colour="red", outlier.shape=19,
                outlier.size=5)+
  annotate("text",  x=16, y=15, size=8, label = paste("n=",ocp_batches$`n (per analyte)`[1] ))+
...
ocp_batches %>%
  ...
  labs(y="Concentration (log10)", x = "Batch number", 
       title =  paste0("Batch Effect Evaluation: OCP"),
       subtitle = "Sample concentration with all of the samples, grouped by analyte") +
  scale_fill_manual(values = vit_c_palette)+
  scale_y_continuous(trans='log10')+
  theme(legend.key.height= unit(5, 'cm'),
        legend.key.width= unit(5, 'cm'),
...
pcb_batches %>% 
  ...
  geom_boxplot( aes(alpha = value),lwd=1.25,outlier.colour="red", outlier.shape=19,
                outlier.size=5)+
  annotate("text",  x=10, y=0.5, size=8, label = paste("n=",pcb_batches$`n (per analyte)`[1] ))+
  facet_wrap(~var, scales = "free",ncol=1)+
  labs(y="Concentration (log10)", x = "Batch number", 
...
pcb_batches %>% 
  ...
  labs(y="Concentration (log10)", x = "Batch number", 
       title =  paste0("Batch Effect Evaluation: PCB"),
       subtitle = "Sample concentration scatterplot, grouped by analyte") +
  scale_fill_manual(values = vit_c_palette)+
  scale_y_continuous(trans='log10')+
  theme(legend.key.height= unit(5, 'cm'),
        legend.key.width= unit(5, 'cm'),
        ...
        axis.title.y = element_text(size = 25),
        axis.title.x = element_text(size = 25))+
        annotate("text",x=15, y=0.01, size=8, label = paste("n=",pcb_batches$`n (per analyte)`[1] ))
...
avg_plot1<- max_min_spiked_ocp %>%
  ...
  annotate("text",x=15, y=2, size=3, label = paste("n=",OCPPBDE_QC_Spiked$`n (per analyte)`[1]))+
  ...
  labs(y="Concentration (log10)", x = "Batch number", 
       ...
       subtitle = "Control chart of OCP, Quality Controls for to see potential fluctuations and interruptions") +
  scale_fill_manual(values = vit_c_palette)+
  scale_y_continuous(trans='log10')+
...
avg_plot2<- max_min_spiked_pcb %>%
  ...
  facet_wrap(~variable, scales="free",ncol=1)+
    geom_blank(aes(y = y_min)) +
    geom_blank(aes(y = y_max))+
  annotate("text",x=10, y=0.2, size=3, label = paste("n=",PCB_QC_Spiked$`n (per analyte)`[1] ))+
  geom_line(aes(color=`Batch_num`)) + 
  geom_point(aes(color=`Batch_num`)) + 
  ...
  labs(y="Concentration (log10)", x = "Batch number", 
       ...
       subtitle = "Control chart of PCB, Quality Controls for to see potential fluctuations and interruptions") +
  scale_fill_manual(values = vit_c_palette)+
  scale_y_continuous(trans='log10')+
  theme(text = element_text(colour = vit_c_palette["light_text"],family = "Cabin"),
        plot.title = element_text(colour = vit_c_palette["dark_text"], #plot title
...
ocp_hline %>%
    filter(str_detect(variable, "HCB"))
...
ocp1<-OCPPBDE_QC_recovery %>% 
  group_by(HCB) %>% 
      ...
      geom_hline(data = ocp_hline, aes(yintercept = 100,linetype = factor(ocp_hline$linetype_mean), 
                                       show_guide = TRUE),linetype="dashed", color = "blue")+
    scale_y_continuous(trans='log10')+
...
### OCP (Spiked) : Scatterplot- all samples
...
ocp_qc_batches$var <- gsub("Results", "", ocp_qc_batches$var) #remove "Results"
...
```{r OCP QC SCatterplot, all samples }
ocp_qc_batches %>%
  group_by(Batch_num) %>% 
  ggplot(aes(x = factor(Batch_num2),  y = value)) +
  geom_line(aes(color=`Batch_num`)) + 
  ...
  facet_wrap(~var,scales = "free", ncol=2)+
  labs(y="Concentration (log10)", x = "Batch number", 
       title = expression("Batch Effect Evaluation: OCP (spiked)"),
       subtitle = expression("QC concentration with all of the samples, grouped by analyte")
       )+
  guides(color="none")+
    geom_hline(data = ocp_qc_hline, aes(yintercept = ocp_qc_hline$,linetype = factor(ocp_hline$linetype_mean), 
                                        show_guide = TRUE),color = "blue", size=0.6)+
  scale_fill_manual(values = vit_c_palette)+
  scale_y_continuous(trans='log10')+
  theme(legend.key.height= unit(5, 'cm'),
        legend.key.width= unit(5, 'cm'),
        ...
        plot.subtitle = element_text(size = rel(2.5), lineheight = 1.3,
                                     margin = margin(0, 0, 1, 0, "lines")),
        strip.text = element_text(colour = vit_c_palette["light_text"], 
                                  size = 25, face = "bold"),
        axis.text=element_text(size=25),
        axis.title.y = element_text(size = 25),
        axis.title.x = element_text(size = 25))
...
grid.arrange(pcb1, ncol=1)
grid.arrange(pcb2,ncol=1)
grid.arrange(pcb3,ncol=1)
grid.arrange(pcb4,ncol=1)
grid.arrange(pcb5,ncol=1)
grid.arrange(pcb6,ncol=1)
grid.arrange(pcb7,ncol=1)
...
shewhart_pcb_unspike<-ggplot(max_min_unspike_pcb, aes(x = factor(Batch_num2), y = value, color=Batch_num)) + 
  ...
  annotate("text",x=10, y=0.00, size=4, label = paste("n=",max_min_unspike_pcb$`n (per analyte)`[1] ))
